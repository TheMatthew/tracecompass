...
import org.eclipse.tracecompass.tmf.ui.widgets.timegraph.ITimeGraphViewerFactory;
 ...
 * @since 2.1
 */
public abstract class AbstractTimeGraphViewStackbars extends TmfView {
    ...
    private SelectionAdapter[] fSelectionAdapters;
    ...
    private class TimeGraphViewerWrapper implements ITimeGraphWrapper {
        ...
        private TimeGraphViewerWrapper(Composite parent, int style) {
            viewer = new TimeGraphViewerStackbars(parent, style);
        }
    ...
    private class TimeGraphViewerFactoryStackbars implements ITimeGraphViewerFactory
    {

        @Override
        public TimeGraphViewer createTimeGraphViewer(Composite parent, int style) {
            // TODO Auto-generated method stub
            return new TimeGraphViewerStackbars(parent, style);
        }
    ...
    private class TimeGraphComboWrapper implements ITimeGraphWrapper {
        ...
        private TimeGraphComboWrapper(Composite parent, int style) {
            combo = new TimeGraphCombo(parent, style, fWeight, new TimeGraphViewerFactoryStackbars());
        }
    ...
    private class ZoomThread extends Thread {
        ...
        public void run() {
            ...
            /*int topIndex = fTimeGraphWrapper.getTimeGraphViewer().getTimeGraphControl().getTopIndex();
            for(int i = topIndex; i < fZoomEntryList.size(); ++i)
            {
                if (fMonitor.isCanceled()) {
                    return;
                }
                zoom(fZoomEntryList.get(i), fMonitor);
            }
     ...
     */
    public AbstractTimeGraphViewStackbars(String id, TimeGraphPresentationProvider pres) {
        super(id);
        fPresentation = pres;
    ...
    protected void setTreeColumns(final String[] columns, SelectionAdapter[] selectionAdapters) {
        fColumns = columns;
        fSelectionAdapters = selectionAdapters;
    }
    ...
    public void createPartControl(Composite parent) {
        if (fColumns == null || fLabelProvider == null) {
            ...
            if(fSelectionAdapters != null)
            {
                combo.setTreeSortColumns(fColumns, fSelectionAdapters);
            }
            else
            {
        ...
        fTimeGraphWrapper.getTimeGraphViewer().addRangeListener(new ITimeGraphRangeListener() {
            ...
            public void timeRangeUpdated(TimeGraphRangeUpdateEvent event) {
                ...
                // TmfTimeRange range = new TmfTimeRange(new
                // TmfNanoTimestamp(startTime), new TmfNanoTimestamp(endTime));
                // broadcast(new
                // TmfRangeSynchSignal(AbstractTimeGraphViewStackbars.this,
                // range));
                if (fZoomThread != null) {
                    fZoomThread.cancel();
                }
        ...
        /*
         * fTimeGraphWrapper.getTimeGraphViewer().addTimeListener(new
         * ITimeGraphTimeListener() {
         *
         * @Override public void timeSelected(TimeGraphTimeEvent event) {
         * TmfNanoTimestamp startTime = new
         * TmfNanoTimestamp(event.getBeginTime()); TmfNanoTimestamp endTime =
         * new TmfNanoTimestamp(event.getEndTime()); broadcast(new
         * TmfTimeSynchSignal(AbstractTimeGraphViewStackbars.this, startTime,
         * endTime)); } });
         */
        ...
        fTimeGraphWrapper.addSelectionListener(new ITimeGraphSelectionListener() {
            ...
            public void selectionChanged(TimeGraphSelectionEvent event) {
                fTimeGraphWrapper.getTimeGraphViewer().getTimeGraphControl().treeViewerSelection(event.getSelection());
     ...
     *
     * /*@TmfSignalHandler public void synchToTime(final TmfTimeSynchSignal
     * signal) { if (signal.getSource() == this || fTrace == null) { return; }
     * final long beginTime = signal.getBeginTime().normalize(0,
     * ITmfTimestamp.NANOSECOND_SCALE).getValue(); final long endTime =
     * signal.getEndTime().normalize(0,
     * ITmfTimestamp.NANOSECOND_SCALE).getValue();
     *
     * Display.getDefault().asyncExec(new Runnable() {
     *
     * @Override public void run() { if (fTimeGraphWrapper.isDisposed()) {
     * return; } if (beginTime == endTime) {
     * fTimeGraphWrapper.getTimeGraphViewer().setSelectedTime(beginTime, true);
     * } else {
     * fTimeGraphWrapper.getTimeGraphViewer().setSelectionRange(beginTime,
     * endTime); }
     * startZoomThread(fTimeGraphWrapper.getTimeGraphViewer().getTime0(),
     ...
     * @param signal The signal that's received
     *
     * @TmfSignalHandler public void synchToRange(final TmfRangeSynchSignal
     * signal) { if (signal.getSource() == this || fTrace == null) { return; }
     * if (signal.getCurrentRange().getIntersection(fTrace.getTimeRange()) ==
     * null) { return; } final long startTime =
     * signal.getCurrentRange().getStartTime().normalize(0,
     * ITmfTimestamp.NANOSECOND_SCALE).getValue(); final long endTime =
     * signal.getCurrentRange().getEndTime().normalize(0,
     * ITmfTimestamp.NANOSECOND_SCALE).getValue();
     * Display.getDefault().asyncExec(new Runnable() {
     *
     * @Override public void run() { if (fTimeGraphWrapper.isDisposed()) {
     * return; }
     * fTimeGraphWrapper.getTimeGraphViewer().setStartFinishTime(startTime,
     * endTime); startZoomThread(startTime, endTime); } }); }
     */
    ...
    protected void refresh() {
        TmfUiRefreshHandler.getInstance().queueUpdate(this, new Runnable() {
            ...
            public void run() {
                ...
                /*
                 * long selectionBeginTime = fTrace == null ? 0 :
                 * fTraceManager.getSelectionBeginTime().normalize(0,
                 * ITmfTimestamp.NANOSECOND_SCALE).getValue(); long
                 * selectionEndTime = fTrace == null ? 0 :
                 * fTraceManager.getSelectionEndTime().normalize(0,
                 * ITmfTimestamp.NANOSECOND_SCALE).getValue(); long startTime =
                 * fTrace == null ? 0 :
                 * fTraceManager.getCurrentRange().getStartTime().normalize(0,
                 * ITmfTimestamp.NANOSECOND_SCALE).getValue(); long endTime =
                 * fTrace == null ? 0 :
                 * fTraceManager.getCurrentRange().getEndTime().normalize(0,
                 * ITmfTimestamp.NANOSECOND_SCALE).getValue(); startTime =
                 * Math.max(startTime, fStartTime); endTime = Math.min(endTime,
                 * fEndTime);
                 */
