...
public class TimeGraphCombo extends Composite {
    ...
    public TimeGraphCombo(Composite parent, int style) {
        this(parent, style, DEFAULT_WEIGHTS, null);
    }
     ...
     * @since 2.1
     */
    public TimeGraphCombo(Composite parent, int style, int[] weights, ITimeGraphViewerFactory timeGraphViewerFactory) {
        super(parent, style);
        ...
        if (timeGraphViewerFactory != null)
        {
            fTimeGraphViewer = timeGraphViewerFactory.createTimeGraphViewer(sash, SWT.NONE);
        }
        else
        {
     ...
     * @param columnNames the tree column names
     */
    public void setTreeSortColumns(String[] columnNames, final SelectionAdapter[] selectionAdapters) {
        final Tree tree = fTreeViewer.getTree();
        for (int i = 0; i < columnNames.length; ++i) {
            final TreeColumn column = new TreeColumn(tree, SWT.LEFT);
            column.setText(columnNames[i]);
            // Adds a listener on the columns header for sorting purpose.
            column.addSelectionListener(selectionAdapters[i]);
            column.pack();
        }
    }
