...
public final class AlignmentParser implements ICommonTreeParser {
     ...
     */
    @Override
    public Long parse(CommonTree node, ICommonTreeParserParameter parser) throws ParseException {
        /*
        ...
        if (node.getType() == CTFParser.CTF_RIGHT) {
            ...
            return parse((CommonTree) node.getChild(0), parser);
        } else if (isUnaryInteger(node)) {
            long alignment = UnaryIntegerParser.INSTANCE.parse(node, null);
