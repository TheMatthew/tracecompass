...
public class SizeParser implements ICommonTreeParser {
     ...
     */
    @Override
    public Long parse(CommonTree rightNode, ICommonTreeParserParameter parser, String error) throws ParseException {
        ...
        if (isUnaryInteger(firstChild)) {
            ...
            long size = UnaryIntegerParser.INSTANCE.parse(firstChild, null, null);
            if (size < 1) {
                throw new ParseException(INVALID_VALUE_FOR_SIZE);
            }
            return size;
