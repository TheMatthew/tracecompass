 ...
 */
@NonNullByDefault
public class CTFPacketReader implements IPacketReader, IDefinitionScope {
    ...
    private final ICTFPacketDescriptor fPacketHeader;
     ...
     * @param packetContext
     *            packet_header where we get info like lost events and cpu_id
     * @param declarations
     ...
     */
    public CTFPacketReader(BitBuffer input, ICTFPacketDescriptor packetContext, Iterable<IEventDeclaration> declarations, IDeclaration eventHeaderDeclaration, @Nullable StructDeclaration streamContext, ICompositeDefinition packetHeader,
            IDefinitionScope packetScope) {
        fInput = input;
        fPacketHeader = packetContext;
        fDeclarations = NonNullUtils.checkNotNull(ImmutableList.copyOf(declarations));
        fHasLost = fPacketHeader.getLostEvents() != 0;
        fLastTimestamp = fPacketHeader.getTimestampBegin();
    ...
    @Override
    public int getCPU() {
        return (int) fPacketHeader.getTargetId();
    }

    @Override
    public boolean hasMoreEvents() {
        return fInput.canRead(1);
    }
    ...
    public EventDefinition readNextEvent() throws CTFException {
         ...
         */
        if (fHasLost && posStart >= fPacketHeader.getContentSizeBits()) {
            fHasLost = false;
            return createLostEvent(fPacketHeader);
        ...
        EventDefinition eventDef = declaration.createDefinition(fStreamContext, fPacketHeader, fTracePacketHeader, fEventHeader, fInput, fLastTimestamp);
    ...
    private EventDefinition createLostEvent(final ICTFPacketDescriptor currentPacket) {
        ...
        IntegerDefinition lostCountDef = new IntegerDefinition(lostEventsDurationDecl, null, CTFStrings.LOST_EVENTS_FIELD, fPacketHeader.getLostEvents());
        IntegerDefinition[] fields = new IntegerDefinition[] { lostCountDef, lostDurationDef };
        int cpu = (int) fPacketHeader.getTargetId();
